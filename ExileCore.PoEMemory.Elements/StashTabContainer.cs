...
public class StashTabContainer : Element
{
	...
	public IList<Element> ViewAllStashPanelChildren => ViewAllStashPanel?.Children.Last((Element x) => x.ChildCount == TotalStashes).Children.Where((Element x) => x.ChildCount > 0).ToList();
