	...
	private unsafe void SetManualFont()
	{
		_overlay.ReplaceFont(delegate(ImFontConfig* config)
		{
			...
			if (Directory.Exists("fonts") && Directory.GetFiles("fonts").Contains("fonts\\config.ini"))
			{
				...
				IntPtr glyphRangesCyrillic = iO.Fonts.GetGlyphRangesCyrillic();
				string[] array2 = array;
				for (int i = 0; i < array2.Length; i++)
				{
					...
					{
						fonts[$"{text.Replace(".ttf", "").Replace("fonts\\", "")}:{num}"] = new FontContainer(iO.Fonts.AddFontFromFileTTF(text, num, config, glyphRangesCyrillic), text, num);
					}
				}
			}
