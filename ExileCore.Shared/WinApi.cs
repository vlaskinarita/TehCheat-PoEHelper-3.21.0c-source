...
public static class WinApi
{
	...
	[Obsolete("Your *really* should be doing this through the IMemory interface")]
	...
	[Obsolete("Your *really* should be doing this through the IMemory interface")]
	public unsafe static bool ReadProcessMemory(IntPtr hWnd, IntPtr baseAddr, byte[] buffer, int size, out IntPtr bytesRead)
	{
		fixed (byte* ptr = buffer)
		{
			void* value = ptr;
			SizeT lpNumberOfBytesRead;
			bool result = Vanara.PInvoke.Kernel32.ReadProcessMemory(hWnd, baseAddr, new IntPtr(value), size, out lpNumberOfBytesRead);
			bytesRead = new IntPtr(lpNumberOfBytesRead);
			return result;
		}
	}
