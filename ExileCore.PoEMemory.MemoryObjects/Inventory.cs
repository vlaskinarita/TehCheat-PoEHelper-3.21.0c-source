...
public class Inventory : Element
{
		...
		get
		{
			...
			case InventoryType.QuadStash:
			{
				foreach (Element child in inventoryUIElement.Children)
				{
					...
					{
						NormalInventoryItem item = child.AsObject<NormalInventoryItem>();
						list.Add(item);
					}
				}
				return list;
			...
			case InventoryType.CurrencyStash:
			{
				...
				if (element2 != null)
				{
					foreach (Element child2 in element2.Children)
					{
			...
			case InventoryType.EssenceStash:
			{
				foreach (Element child4 in inventoryUIElement.Children)
				{
					...
					{
						list.Add(child4[1].AsObject<EssenceInventoryItem>());
					}
				}
				return list;
			}
			case InventoryType.FragmentStash:
			{
				...
				{
					element3 = base.Children[0];
				}
				else if (base.Children[1].IsVisible)
				{
					element3 = base.Children[1];
				}
				else if (base.Children[2].IsVisible)
				{
					element3 = base.Children[2];
				}
				...
				{
					foreach (Element child5 in element3.Children)
					{
						if (child5.ChildCount > 1)
						{
							list.Add(child5[1].AsObject<FragmentInventoryItem>());
						}
					}
					return list;
				}
			...
			case InventoryType.MapStash:
				...
				foreach (Element child6 in base.Children[3].Children)
				{
					if (!child6.IsVisible || child6.ChildCount <= 1)
					{
						continue;
					}
					for (int j = 1; j < child6.ChildCount; j++)
					{
						Element element = child6.Children[j];
						if (element.ChildCount == 1)
						{
							list.Add(element.AsObject<NormalInventoryItem>());
						}
					}
				}
			...
			case InventoryType.DelveStash:
			{
				foreach (Element child7 in inventoryUIElement.Children)
				{
					...
					{
						list.Add(child7[1].AsObject<DelveInventoryItem>());
					}
				}
				return list;
			}
			case InventoryType.BlightStash:
			{
				foreach (Element child8 in inventoryUIElement.Children)
				{
					...
					{
						list.Add(child8[1].AsObject<BlightInventoryItem>());
					}
				}
				return list;
			}
			case InventoryType.DeliriumStash:
			{
				foreach (Element child9 in inventoryUIElement.Children)
				{
					...
					{
						list.Add(child9[1].AsObject<DeliriumInventoryItem>());
					}
				}
				return list;
			}
			case InventoryType.MetamorphStash:
			{
				...
				{
					if (child10.ChildCount > 1)
					{
						list.Add(child10[1].AsObject<MetamorphInventoryItem>());
					}
				}
				return list;
			}
			case InventoryType.UniqueStash:
			{
				...
				{
					if (child11.ChildCount > 1)
					{
						list.Add(child11[1].AsObject<NormalInventoryItem>());
					}
				}
				return list;
			}
	...
	public Inventory()
	{
		_cachedValue = new FrameCache<InventoryOffsets>(() => base.M.Read<InventoryOffsets>(GetChildAtIndex(0)?.Address ?? 0));
