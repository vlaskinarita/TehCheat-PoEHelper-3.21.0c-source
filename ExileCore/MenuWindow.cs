...
public class MenuWindow : IDisposable
{
	...
	{
		List<PluginWrapper> list = core.pluginManager?.Plugins.OrderBy((PluginWrapper x) => x.Name).ToList() ?? new List<PluginWrapper>();
		...
		if (_CoreSettings.MainMenuKeyToggle.PressedOnce())
		{
			...
			{
				_settingsContainer.SaveCoreSettings();
				if (gameController != null && core.pluginManager != null)
				{
					foreach (PluginWrapper plugin in core.pluginManager.Plugins)
					{
		...
		using (core.Graphics.UseCurrentFont())
		{
			...
			ImGui.BeginChild("Left menu window", new System.Numerics.Vector2(PluginNameWidth, ImGui.GetContentRegionAvail().Y), border: true, ImGuiWindowFlags.None);
			...
			if (gameController != null && core.pluginManager != null)
			{
				...
				{
					PluginWrapper pluginWrapper = list[i];
					bool v = pluginWrapper.IsEnable;
					if (ImGui.Checkbox($"##{pluginWrapper.Name}{i}", ref v))
					{
						pluginWrapper.TurnOnOffPlugin(v);
					}
					ImGui.SameLine();
					if (ImGui.Selectable(pluginWrapper.Name, _index == i))
					{
						_index = i;
						Selected = pluginWrapper.DrawSettings;
					}
				}
				int num = -2;
				foreach (KeyValuePair<string, string> failedSourcePlugin in core.pluginManager.FailedSourcePlugins)
				{
					...
					bool flag = _index == --num;
					bool num2 = ImGui.Selectable(pluginName, flag);
					ImGui.PopStyleColor();
					if (num2 || flag)
					{
						_index = num;
			...
			ImGui.EndChild();
			ImGui.SameLine();
			ImGui.BeginChild("Options", ImGui.GetContentRegionAvail(), border: true);
			Selected?.Invoke();
			ImGui.EndChild();
			ImGui.End();
		}
	...
	private void DebugWindowRender()
	{
		...
		case Windows.Caches:
			if (ImGui.BeginTable("Cache table", 6, ImGuiTableFlags.Borders))
			{
				...
				(string, IStaticCache)[] array = new(string, IStaticCache)[6]
				{
					("Elements", cache.StaticCacheElements),
					("Components", cache.StaticCacheComponents),
					("Entity", cache.StaticEntityCache),
					("Entity list parse", cache.StaticEntityListCache),
					("Server Entity list parse", cache.StaticServerEntityCache),
					("String cache", cache.StringCache)
				};
	...
	private void DrawMainDebugInfoLine(MainDebugTableRecord debug, float maxTotal)
	{
		...
		ImGui.TableNextColumn();
		ImGui.TextColored(col, $"{debug.Total:0.000}".PadLeft((int)Math.Floor(Math.Log10(maxTotal)) + 5));
		ImGui.TableNextColumn();
