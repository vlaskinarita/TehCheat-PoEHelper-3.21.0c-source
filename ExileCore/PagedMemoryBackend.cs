...
public class PagedMemoryBackend : IMemoryBackend, IDisposable
{
	...
	{
		return (IntPtr)(long)(address.GetValue() / 4096uL * 4096);
	}
	...
	public bool TryReadMemory(IntPtr address, Span<byte> target)
	{
		...
		try
		{
			...
			IntPtr pageAddress2 = GetPageAddress(address + target.Length - 1);
			if (pageAddress.GetValue() > pageAddress2.GetValue())
			{
				return false;
			}
			int num = checked((int)(address.GetValue() - pageAddress.GetValue()));
			IntPtr intPtr = pageAddress;
			Memory<byte> memory;
			Span<byte> span;
			while (intPtr.GetValue() < pageAddress2.GetValue())
			{
				IMemoryOwner<byte> page = GetPage(intPtr);
				if (page == null)
				{
					return false;
				}
				...
				span = memory.Span;
				int num2 = num;
				span.Slice(num2, 4096 - num2).CopyTo(target);
				intPtr += 4096;
				num2 = 4096 - num;
				target = target.Slice(num2, target.Length - num2);
				num = 0;
			}
	...
	public void NotifyFrame()
	{
		...
		try
		{
			...
			Task.Run(delegate
			{
				foreach (var (intPtr, num) in pagesToPreRead)
				{
					if (thisFrameCts.IsCancellationRequested)
					{
						break;
					}
					using IMemoryOwner<byte> memoryOwner = GetRealPage(intPtr, num);
					if (thisFrameCts.IsCancellationRequested)
					{
						break;
					}
					if (memoryOwner != null)
					{
						for (int i = 0; i < num * 4096; i += 4096)
						{
							...
							try
							{
								if (_disposed || !_cachedPages.TryAdd(intPtr + i, memoryOwner2))
								{
