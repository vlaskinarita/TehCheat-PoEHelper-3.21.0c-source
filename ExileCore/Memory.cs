...
public class Memory : IMemory, IDisposable
{
	...
	public string ReadString(long addr, int length = 256, bool replaceNull = true)
	{
		...
		string @string = Encoding.ASCII.GetString(span2);
		if (!replaceNull)
		{
			return @string;
		}
	...
	{
		if (lengthBytes > 5120 || lengthBytes < 0)
		{
			return string.Empty;
		}
		if (addr == 0L)
	...
	public T[] ReadStdVectorStride<T>(NativePtrArray nativeContainer, int stride) where T : unmanaged
	{
		...
		long num2 = nativeContainer.Last - nativeContainer.First;
		long num3 = nativeContainer.ElementCount(stride);
	...
	public long[] FindPatterns(params IPattern[] patterns)
	{
		...
		void FindPattern(long iPattern)
		{
			...
			{
				for (int k = pattern2.StartOffset; k < exeImage.Length - num; k++)
				{
				...
				if (!flag)
				{
					for (int l = 0; l < pattern2.StartOffset; l++)
					{
						if (CompareData(pattern2, exeImage.AsSpan(l)))
						{
							address[iPattern] = l;
							break;
						}
